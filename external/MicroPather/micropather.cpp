...
void OpenQueue::Push(PathNode* pNode)
{
    ...
    // Add sorted. Lowest to highest cost path. Note that the sentinel has a value of FLT_MAX, so it should always be sorted in.
...
void OpenQueue::Update(PathNode* pNode)
{
    ...
    // If the node now cost less than the one before it, move it to the front of the list.
      ...
      nAvailable(0)
{
    ...
    // Want the behavior that if the actual number of states is specified, the cache will be at least that big.
    hashShift = 3;    // 8 (only useful for stress testing)
#ifndef MICROPATHER_STRESS
...
PathNode* PathNodePool::GetPathNode(unsigned int frame,
    void* _state,
...
void MicroPather::GetNodeNeighbors(PathNode* node, MP_VECTOR<NodeCost>* pNodeCost)
{
    ...
    else if (node->cacheIndex < 0)
    {
        ...
        if (node->numAdjacent > 0)
        {
            ...
            const unsigned int stateCostVecSize = stateCostVec.size();
            ...
            // Can this be cached?
            int start = 0;
            if (!pNodeCost->empty() && pathNodePool.PushCache(pNodeCostPtr, pNodeCost->size(), &start))
...
{
    // Important to clear() in case the caller doesn't check the return code. There can easily be a leftover path  from a previous call.
    path->clear();
